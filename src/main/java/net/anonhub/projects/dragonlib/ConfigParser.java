...
import net.anonhub.projects.dragonlib.configParser.noValueException.*;
import net.anonhub.projects.dragonlib.configParser.*;
import net.anonhub.projects.dragonlib.configParser.noValueException.*;
...
public class ConfigParser {
    ...
    public ConfigParser(String filePath, HashMap<String, HashMap<String, ArrayList<String>>> defaultSettings) {
        file = Paths.get(filePath);
        path = file.getParent();
        try {
            read();
        } catch (BadPathException ignore) {}
        update(defaultSettings);
        writeFile();
    }
    public ConfigParser(Path filePath, HashMap<String, HashMap<String, ArrayList<String>>> defaultSettings) {
        file = filePath;
        path = file.getParent();
        try {
            read();
...
//                                  TODO add breakout characters
                                    break;
                                }
                            }
                            if (!subed) {
                                throw new MissingEndQuote("Set \"" + setName + "\" is missing an end quote in setting \""+name+"\"");
                            }
                        }
                        case ('\'') -> {
                            valueTypes.add("c");
                            ...
                            if (!subed) {
                                throw new MissingEndSingleQuote("Set " + rawSet + " is missing an end single quote in setting \""+name+"\"");
                            }
                        }
                        case ('b') -> {
                            valueTypes.add("b");
                        ...
                        default -> throw new IllegalValueException("Unknown data type: "+rawValues.toString().split(",")[0]);
                    }
                }
...
//                add set to setting
    ...
    public void writeFile() {
        if (Files.notExists(file)) {
            ...
            throw new BadPathException("There is a directory where their should be a file. Fix it before trying again.");
        }
            ...
            for (String key1:settings.get(key).keySet()) {
                ...
                for (int i = 0; i < settings.get(key).get(key1).size(); i++) {
                    switch (settings.get(key).get(key1+DATA_TYPE_CHAR).get(i)) {
                        ...
                        case "f" -> sb.append('f');
                        case "d" -> sb.append('d');
                        default -> throw new IllegalValueException("Unknown dataType: "+settings.get(key).get(key1+DATA_TYPE_CHAR).get(i));
                    }
